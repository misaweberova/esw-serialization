...
MeasurementInfo Result::getInfo() const { return info; }

void Result::setInfo(const MeasurementInfo &value) { info = value; }

std::map<DataType, double> Result::getAverages() const { return averages; }
...
Result::Result(MeasurementInfo info,
               const std::map<DataType, std::vector<double>> &data)
{
   this->info = info;
